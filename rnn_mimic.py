...
def return_data(synth_data=False, balancer=True, target='VANCOMYCIN', 
...
def build_model(no_feature_cols=None, time_steps=7, output_summary=False):
  ...
  input_layer = tf.keras.Input(shape=(time_steps, no_feature_cols)) 
...
def train(model_name="kaji_mach_0", synth_data=False, target='VANCOMYCIN',
  ...
  log_directory = "./logs/{0}_{1}.log".format(model_name, time)
...
def pickle_objects(target='VANCOMYCIN', time_steps=14):
...
# VANCOMYCIN ##
    ...
    train(model_name='kaji_mach_final_no_mask_VANCOMYCIN_pad14_20_percent',
          epochs=17, synth_data=False, predict=True, target='VANCOMYCIN',
